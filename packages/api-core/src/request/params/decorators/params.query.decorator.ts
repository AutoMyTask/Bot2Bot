...
export class ParamsQueryDecorator
  extends ParamsDecorator<
  ...
  implements RequestCore.Params.IParamsQueryDecorator
{
  ...
  add(
    index: number,
    option: { required?: boolean; name: string; type?: "int" | "float" },
  ): void {
    const typeConstruct = this.types[index];

    if (typeConstruct !== Number && typeConstruct !== String) {
      throw new Error(
        `Invalid parameter type for '${option.name}' in method '${
          this.methodName as string
        }'. The parameter should be a Number or a String, but a ${
          typeConstruct.name
        } was provided.`,
      );
    }
    ...
    if (
      ...
      throw new Error(
        `Invalid parameter type for '${option.name}' in method '${
          this.methodName as string
        }'. The parameter should be a Number, but a ${
          typeConstruct.name
        } was provided. 'int' or 'float' can only be associated with a 'Number' type for this specific parameter.`,
      );
    }
    const type =
      option?.type ??
      (typeConstruct.name.toLowerCase() as RequestCore.Params.ParamQueryType);
