...
import { RouteCore, TypesCore } from "api-core-types";

export class GroupedRouteBuilder
  extends BaseRouteBuilder
  implements RouteCore.IGroupedEndpointRouteBuilder, RouteCore.IRouteMapBuilder
{
  ...
  constructor(
    ...
    if (routeMapBuilder instanceof GroupedRouteBuilder) {
      ...
      this.metadataCollection.items.push(
        ...routeMapBuilder.metadataCollection.items,
      );
    }
    ...
    if (!/^\/([^/]+(\/[^/]+)*|[^/]+)$/.test(prefix)) {
      throw new Error(
        `Invalid prefix format for '${prefix}'. Please use '/{string}/...' format.`,
      );
    }
  }
  ...
  buildRouteConventions(): RouteCore.IRouteConventions[] {
    const routeConventionsSubRoute = this.routesBuilders.reduce(
      (requestsHandlersConventions, routeBuilder) => {
        ...
        for (let routeConvention of routeConventions) {
          ...
          routeConvention.metadataCollection.items.push(
